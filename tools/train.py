...
def main():
    ...
    check_config(cfg)
    check_gpu(cfg.use_gpu)      # check if set use_gpu=True in paddlepaddle cpu version
    check_version()     # check if paddlepaddle version is satisfied
    ...
    train_reader = create_reader(cfg.TrainReader, (cfg.max_iters - start_iter) * devices_num, cfg,
                                 devices_num=devices_num, num_trainers=num_trainers)
    ...
    # windows 中不要安装 visualdl，会导致环境崩溃
    if FLAGS.use_vdl:
    ...
    for it in range(start_iter, cfg.max_iters):
        ...
        if (it > 0 and it % cfg.snapshot_iter == 0 or it == cfg.max_iters - 1) and (not FLAGS.dist or trainer_id == 0):
            ...
            if FLAGS.eval:
                ...
                results = eval_run(exe, compiled_eval_prog, eval_loader, eval_keys, eval_values, eval_cls,
